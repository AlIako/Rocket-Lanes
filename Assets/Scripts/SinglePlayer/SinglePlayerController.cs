...
public class SinglePlayerController : MonoBehaviour, INetworkController
{
	...
	{
		//spawn player
		Player player1 = Instantiate(playerPrefab, gameController.GetLane(0).startPosition.transform.position, Quaternion.identity);
		...
		for(int i = 0; i < 3; i++)
		{
			Player playerAI = Instantiate(playerPrefab, gameController.GetLane(1 + i).startPosition.transform.position, Quaternion.identity);
	...
	{
		int randomIndex = gameController.GetLane(toPlayerId).spawnManager.GetRandomSpawnerIndex();
		gameController.GetLane(toPlayerId).spawnManager.Spawn(randomIndex);
	}
    ...
    public int AskForConsent(ConsentAction consentAction, int[] parameters)
	{
		if(consentAction == ConsentAction.SpawnRocket)
		{
			...
			return gameController.GetLane(parameters[1]).spawnManager.GetRandomSpawnerIndex();
		}
		return -1;
	}
